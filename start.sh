...
echo This is the no bbreg version with func space
...
fun_runs=$(find "${data_dir}/derivatives/fmriprep/sub-$subj/ses-7t01/func" -type f -name "*run*_desc-preproc_bold.nii.gz" | sort -t '\0' -n)
...
mypRFdir="${data_dir}/derivatives/prfVista/sub-$subj/ses-$ses/"
mkdir -p $mypRFdir
